...
### Added
- **Managed dev server** (`bin/dev-server`): single fixed-port PHP dev server
  (default `127.0.0.1:8010`) with `start`/`stop`/`restart`/`status` subcommands.
  `start` reclaims the port from any stray `php -S` bound to it, so we stop
  accumulating orphaned dev servers across sessions. Override the port with
  `MONSTER_DEV_PORT`.
